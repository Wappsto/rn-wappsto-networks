...
const Button = React.memo(
  ({ text, icon, style, color, bold, onPress, align, type, request, display, ...props }) => {
    const textColor = () => {
      if (props.disabled) {
    ...
    return (
      <TouchableOpacity
        ...
        {icon && !request && (
          <Icon
            style={[text ? styles.iconWithText : styles.icon, type && { color: textColor() }]}
            name={icon}
          />
        )}
        {request && request.status === 'pending' && (
          <ActivityIndicator
            style={[text ? styles.iconWithText : styles.icon, { color: textColor() }]}
            size="small"
        ...
        {text && (
          <Text
            style={[
              styles.text,
              bold &&
                (theme.variables.fontFamilyBold
